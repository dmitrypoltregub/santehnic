...
if($_REQUEST['action'] == 'print_label_ozon'){
    if(is_set($_REQUEST['orders'])) {
        ...
        if($res['error']) echo \Bitrix\Main\Web\Json::encode($res);
        else  return $res;
    }
    else
...
if($_REQUEST['action'] == 'get_documents'){
    $task = CMaxyssOrderList::GetDocuments(htmlspecialcharsbx($_REQUEST['ozon_id']));
        ...
        if($arIblock = $bdIblock->Fetch())
        {
            ...
            if($ob = $res_el->GetNextElement()) {
                ...
                if(is_array($iblock_info)) {
                    if ($iblock_info['PRODUCT_IBLOCK_ID'] == intval($_REQUEST["iblock_id"])) {
                        $iblock_id = intval($_REQUEST["iblock_id"]);
...
//                                }else $custom_values = '';
                        ...
                        */
                        ...
                        foreach ($prop_sinc_values_select_array as $prop){
                            if(isset($prop['UF_XML_ID']))
                                $custom_values_table .= '<tr><td><input type="hidden" value="'.$prop["UF_XML_ID"]. '" name="prop_value['.$iblock_id.']['.$index.']">'.$prop['VALUE'].'</td><td><select name="attr_value[' . intval($_REQUEST['iblock_id']) . ']['.$index.']"><option value=""></option>';
                            else
    ...
    //                                    $prop_sinc_values_select .= '<option value="' . $enum_fields["ID"] . '">' . '[' . $enum_fields["ID"] . '] ' . $enum_fields["VALUE"] . '</option>';
                            ...
                            foreach ($prop_sinc_values_select_array as $prop){
                                ...
                                if(isset($prop['UF_XML_ID']))
                                    $custom_values_table .= '<tr><td><input type="hidden" value="'.$prop["UF_XML_ID"]. '" name="prop_value['.$iblock_offers_id.']['.$index.']">'.$prop['UF_NAME'].'</td><td><select name="attr_value[' . intval($_REQUEST['iblock_id']) . ']['.$index.']"><option value=""></option>';
                                else
